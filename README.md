...
Статус: **Beta (v1.0.0-beta.1)**. Обнаружение, сопряжение с
подтверждением, синк метаданных, передача аудиофайлов и обложек —
всё проверено вживую на двух реальных устройствах (Linux + Android) и
работает. Дедупликация — пока только грубая эвристика (не
`chromaprint`). Подробности и явные ограничения:
...
- Устройства: список сопряжённых узлов с online/offline, QR-код id
  устройства, автоматический синк с уже сопряжёнными при обнаружении в
  LAN, бейдж "синхронизировано N изменений". Сопряжение — только по
  явному подтверждению с обеих сторон ("Найдено рядом" → "Добавить" →
  диалог "Разрешить/Отклонить" на другом устройстве); отвязать
  устройство можно в любой момент (docs/adr/0011).
...
- Профили: несколько человек могут делить одно устройство — у каждого
  ...
  локальная БД на профиль). Переключение — карточка профиля на вкладке
  "Устройства" → "Сменить" (docs/adr/0013).
