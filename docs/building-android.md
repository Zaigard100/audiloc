...
| Разрешение | Зачем |
|---|---|
| `INTERNET`, `ACCESS_NETWORK_STATE` | REST-запросы к Syncthing (`localhost:8384`) и вообще сеть |
...
- **Не проверялось живой сборкой/запуском** в среде разработки — нет
  Android SDK. Инструкция собрана по документации Flutter/плагинов, а
  не проверена эмпирически (в отличие от Linux-сборки, которая
  реально собиралась и тестировалась — см. `docs/roadmap.md`).
...
- **Foreground-сервис для Syncthing на Android** (ТЗ п.9) — не
  начато; `SyncthingProcessManager` в текущем виде рассчитан на
  десктоп (`Process.start`), на Android так внешний процесс не
  запустить — нужен форк вроде syncthing-android как референс.
